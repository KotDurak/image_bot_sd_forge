...
async def get_user_settings(user_id: int) -> dict:
    ...
    cursor = await async_db.conn.execute(
        "SELECT model, preset, requests_count, vae FROM user_settings WHERE user_id = ?",
        (user_id,)
    )
    row = await cursor.fetchone()

    if not row:
        return {"model": None, "preset": None, "requests_count": 0, "vae": None}
...
async def update_user_settings(user_id: int, username: str = None, **kwargs):
    ...
    valid_fields = {'username', 'model', 'preset', 'requests_count', 'vae', 'last_request_at'}
    updates = []
    params = []
